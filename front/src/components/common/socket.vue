    ...
    sockets: {
        ...
        newMsg: function(res) {
            if (this.$store.state.user.isLogin && res.from !== this.$store.state.user.name) {
                ...
                this.$store.state.sessions.sort(booleanCompare('hasNewMsg'))
                console.log('newMsg', this.$store.state.sessions)
            }
        },
        ...
        newCheck: function(res) { // 新的审核通知
            if (res.data.username === this.$store.state.user.name) {
                this.$notify({
                    title: `审核通知`,
                    position: 'bottom-right',                    
                    message: `您的管理员申请结果如下:【${res.data.isPass ? '审核已通过' : '审核被拒绝'}】`,
                    duration: 0,
